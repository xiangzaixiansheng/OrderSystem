...
import (
	"OrderSystem/models"
	"OrderSystem/services/mq"
	redisClient "OrderSystem/services/redis"
	"OrderSystem/utils"
	"encoding/json"
	"fmt"
	"math/rand"
...
func (this *OrderController) Seckill() {
	...
	if skuId == 0 {
		this.Data["json"] = utils.ReturnError(4001, "sku id不能为空")
		this.ServeJSON()
		return
	}
	if address == "" {
		this.Data["json"] = utils.ReturnError(4002, "收货地址不能为空")
		this.ServeJSON()
		return
	}
	...
	if err != nil {
		this.Data["json"] = utils.ReturnError(4003, "该商品不存在")
		this.ServeJSON()
		return
	}
	if stock < 1 {
		this.Data["json"] = utils.ReturnError(4005, "该商品库存不足")
		this.ServeJSON()
		return
	}
	...
	if err != nil {
		this.Data["json"] = utils.ReturnError(4006, "商品秒杀时间缓存不存在")
		this.ServeJSON()
		return
		...
		if startTime.After(now) {
			this.Data["json"] = utils.ReturnError(4007, "秒杀还未开始")
			this.ServeJSON()
			return
		}
	...
	if err == nil {
		this.Data["json"] = utils.ReturnError(4008, "该用户已经下过单"+order)
		this.ServeJSON()
		return
	}
	...
	this.Data["json"] = utils.ReturnSuccess(2000, "秒杀中", nil, 0)
	this.ServeJSON()
	return
}
